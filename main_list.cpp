...
int main(int argc, char **argv) {
    ...
    // Start timing the bracket checking process using the list-based stack
    auto start = high_resolution_clock::now();

    // Loop through all provided file names to check each one
    for (int i = 1; i < argc; ++i) {
        // Using the checkWithList method to validate brackets in each file
        if (checker.checkWithList(argv[i]) == 1) {
    ...
    cout << "Checking all files took " << duration.count() << " microseconds with list-based stack\n";
